...
/**
  ******************************************************************************
  * @file           : main.cpp
  * @brief          : ROS rosserial temporary encoderless DC motor controller
  ******************************************************************************
  */
...
static void apply_open_loop_limited_pid_outputs(void)
{
   ...
   */
  reset_pid();
  desired_left_pwm = base_left_pwm;
  desired_right_pwm = base_right_pwm;
...
static void PublishEncoderTicks(void)
{
  /* No encoder feedback is used in this temporary DC mode.
   * Publish zeros so downstream tools do not mistake floating encoder pins
   * for valid wheel odometry.
   */
  wheel_ticks_msg.x = 0.0f;
  wheel_ticks_msg.y = 0.0f;
...
int main(void)
{
  ...
  nh.loginfo("STM32 ROS Motor Control Ready - Encoderless DC test mode");
