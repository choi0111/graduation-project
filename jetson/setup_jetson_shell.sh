...
RUN_RVIZ_LOCAL_SCRIPT="${PROJECT_DIR}/jetson/run_rviz_local.sh"
BASHRC="${HOME}/.bashrc"
...

{
  ...
  echo "alias rviz_nav='bash ${RUN_RVIZ_LOCAL_SCRIPT}'"
  echo "${MARKER_END}"
} >> "${TMP_BASHRC}"
...
echo "After that, run: autodrive, dc_autodrive, robot_start, or rviz_nav"
